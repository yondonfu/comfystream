...
logging.basicConfig(level=logging.INFO, format="[ComfyStream] %(message)s", stream=sys.stdout)
...
class LocalComfyStreamServer(ComfyStreamServerBase):
    ...
    async def start(self, port=None, host=None):
        ...
        try:
            ...
            threading.Thread(
                target=self.log_subprocess_output, args=(self.process.stdout, logging.INFO)
            ).start()
            threading.Thread(
                target=self.log_subprocess_output, args=(self.process.stderr, logging.ERROR)
            ).start()
            ...
            for _ in range(self.health_check_timeout):
                ...
                raise RuntimeError(
                    f"Server failed to start after {self.health_check_timeout} seconds"
                )
            ...
            logging.info(
                f"ComfyStream server started on port {self.port} (PID: {self.process.pid})"
            )
            return True
