...
async def cache_control_middleware(request, handler):
    ...
    if request.path == target_path:
        ...
        logging.debug(
            f"[CacheMiddleware] Added Cache-Control: no-cache for {request.path}"
        )  # Kept debug log
    return response
...
if hasattr(PromptServer.instance, "routes") and hasattr(PromptServer.instance.routes, "static"):
    ...
    try:
        ...
        logging.warning(
            f"Failed to get extension name dynamically: {e}, using fallback '{extension_name}'"
        )
    ...
    if hasattr(PromptServer.instance, "app"):
        ...
        logging.warning(
            "Could not add ComfyStream cache control middleware: PromptServer.instance.app not found."
        )
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if not response.ok:
                        return web.json_response(
                            {"error": f"Server error: {response.status}"}, status=response.status
                        )
    ...
    async def control_server(request):
        ...
        try:
            ...
            if action == "status":
                ...
                return web.json_response({"success": True, "status": server_manager.get_status()})
            elif action == "start":
                success = await server_manager.start(port=port, host=host)
                return web.json_response(
                    {"success": success, "status": server_manager.get_status()}
                )
            elif action == "stop":
                try:
    ...
    async def update_settings(request):
        ...
        try:
            ...
            return web.json_response(
                {"success": success, "settings": settings_storage.load_settings()}
            )
        except Exception as e:
