...
if hasattr(PromptServer.instance, "routes") and hasattr(
    ...
    STATIC_DIR = (
        pathlib.Path(__file__).parent.parent.parent / "nodes" / "web" / "static"
    )
    ...
    routes.static(
        STATIC_ROUTE, str(STATIC_DIR), append_version=False, follow_symlinks=True
    )
    ...
    async def control_server(request):
        ...
        try:
            ...
            if action == "status":
                ...
                return web.json_response(
                    {"success": True, "status": server_manager.get_status()}
                )
            elif action == "start":
    ...
    async def manage_configuration(request):
        ...
        try:
            ...
            if action == "add":
                ...
                if index is None:
                    return web.json_response(
                        {"error": "Missing index parameter"}, status=400
                    )
