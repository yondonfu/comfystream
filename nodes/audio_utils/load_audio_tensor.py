...
from comfystream.exceptions import ComfyStreamAudioBufferError, ComfyStreamInputTimeoutError
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "buffer_size": (
                    "FLOAT",
                    {"default": 500.0, "tooltip": "Audio buffer size in milliseconds"},
                ),
            },
            "optional": {
                "timeout_seconds": (
            ...
            while total_samples < self.buffer_samples:
                try:
                    ...
                    if frame.sample_rate != self.sample_rate:
                        raise ValueError(
                            f"Sample rate mismatch: expected {self.sample_rate}Hz, got {frame.sample_rate}Hz"
                        )
                    chunks.append(frame.side_data.input)
                    total_samples += frame.side_data.input.shape[0]
                except queue.Empty:
                    raise ComfyStreamAudioBufferError(
                        timeout_seconds, self.buffer_samples, total_samples
                    )

            merged_audio = np.concatenate(chunks, dtype=np.int16)
            buffered_audio = merged_audio[: self.buffer_samples]
            self.leftover = (
                merged_audio[self.buffer_samples :]
