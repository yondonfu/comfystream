...
async function controlServer(action) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        "[ComfyStream] Server returned error response:",
        response.status,
        errorText,
      );
      try {
    ...
    app.ui.dialog.show(
      "Error",
      error.message || "Failed to control ComfyStream server",
    );
    throw error;
  }
}
...
async function openUI() {
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error(
        "[ComfyStream] UI info returned error response:",
        response.status,
        errorText,
      );
      try {
    ...
    if (!data.success) {
      throw new Error(
        data.error || "Unknown error getting ComfyStream UI info",
      );
    }
    ...
    app.ui.dialog.show(
      "Error",
      error.message || "Failed to launch ComfyStream",
    );
    throw error;
  }
}
...
const extension = {
  ...
  setup() {
    ...
    try {
      if (
        app.ui.settings &&
        app.ui.settings.store &&
        typeof app.ui.settings.store.get === "function"
      ) {
        useNewMenu = app.ui.settings.store.get("Comfy.UseNewMenu") || "Enabled";
      }
    } catch (e) {
      console.log(
        "[ComfyStream] Could not access settings store, defaulting to new menu system",
      );
    }
    ...
    if (useNewMenu === "Disabled") {
      ...
      comfyStreamMenu.addItem("Open UI", openUI, {
        icon: "pi pi-external-link",
      });
      comfyStreamMenu.addSeparator();
      comfyStreamMenu.addItem("Start Server", () => controlServer("start"), {
        icon: "pi pi-play",
      });
      comfyStreamMenu.addItem("Stop Server", () => controlServer("stop"), {
        icon: "pi pi-stop",
      });
      comfyStreamMenu.addItem(
        "Restart Server",
        () => controlServer("restart"),
        { icon: "pi pi-refresh" },
      );
      comfyStreamMenu.addSeparator();
      comfyStreamMenu.addItem("Server Settings", openSettings, {
        icon: "pi pi-cog",
      });
    }
