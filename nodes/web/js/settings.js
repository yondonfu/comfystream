...
class ComfyStreamSettings {
  ...
  async loadSettings() {
    try {
      ...
      console.error(
        "[ComfyStream Settings] Error loading settings from server:",
        error,
      );
      ...
      try {
        ...
        if (savedSettings) {
          ...
          this.saveSettings().catch((e) => {
            console.error(
              "[ComfyStream Settings] Failed to save localStorage settings to server:",
              e,
            );
          });
        }
        ...
        console.error(
          "[ComfyStream Settings] Error loading settings from localStorage:",
          localError,
        );
      }
    ...
    try {
      ...
      try {
        localStorage.setItem(
          "comfystream_settings",
          JSON.stringify(this.settings),
        );
      } catch (localError) {
        console.error(
          "[ComfyStream Settings] Error saving to localStorage:",
          localError,
        );
      }
      ...
      console.error(
        "[ComfyStream Settings] Error saving settings to server:",
        error,
      );
  ...
  async addConfiguration(name, host, port) {
    try {
      ...
      console.error(
        "[ComfyStream Settings] Error adding configuration:",
        error,
      );
  ...
  async removeConfiguration(index) {
    try {
      ...
      console.error(
        "[ComfyStream Settings] Error removing configuration:",
        error,
      );
  ...
  async selectConfiguration(index) {
    try {
      ...
      console.error(
        "[ComfyStream Settings] Error selecting configuration:",
        error,
      );
  ...
  getSelectedConfigName() {
    if (
      this.settings.selectedConfigIndex >= 0 &&
      this.settings.selectedConfigIndex < this.settings.configurations.length
    ) {
      return this.settings.configurations[this.settings.selectedConfigIndex]
        .name;
    }
    return null;
  }
  ...
  async function updateConfigsList() {
    ...
    settingsManager.settings.configurations.forEach((config, index) => {
      ...
      selectButton.textContent =
        index === settingsManager.settings.selectedConfigIndex
          ? "Selected"
          : "Select";
    ...
    // Add event listeners
    document
      .querySelectorAll(".comfystream-config-select")
      .forEach((button) => {
        if (!button.disabled) {
          button.addEventListener("click", async (e) => {
            ...
            const selectedName = settingsManager.getSelectedConfigName();
            currentConfigName.textContent = selectedName || "Custom (unsaved)";
            currentConfigName.style.fontStyle = selectedName
              ? "normal"
              : "italic";
    ...
    document
      .querySelectorAll(".comfystream-config-delete")
      .forEach((button) => {
        button.addEventListener("click", async (e) => {
          const index = parseInt(e.target.dataset.index);
