    ...
    async def get_available_nodes(self):
        ...
        try:
            ...
            for prompt_index, prompt in enumerate(self.current_prompts):
                ...
                needed_class_types = {
                    node.get("class_type") for node in prompt.values()
                }
                ...
                for class_type, node_class in nodes.NODE_CLASS_MAPPINGS.items():
                    if (
                        not remaining_nodes
                    ):  # Exit early if we've found all needed nodes
                        break
                        ...
                        if "inputs" in node:
                            for input_name, input_value in node["inputs"].items():
                                node_info["inputs"][input_name] = {
                                    "value": input_value,
                                    "type": input_info.get(input_name, {}).get(
                                        "type", "unknown"
                                    ),
                                    "min": input_info.get(input_name, {}).get(
                                        "min", None
                                    ),
                                    "max": input_info.get(input_name, {}).get(
                                        "max", None
                                    ),
                                    "widget": input_info.get(input_name, {}).get(
                                        "widget", None
                                    ),
                                }
